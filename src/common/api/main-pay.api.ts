...
export class MainPayApi {
    ...
    private static makeTimestamps() {
        return `${format(new Date(), 'yyyyMMddHHmmss',)}${MainPayApi.generateRandomNumber(4)}`;
    };
     ...
     */
    public async billKeyRegister(data: IBillKeyRegisterParams): Promise<IBillKeyRegisterResponse> {
     ...
     */
    public async billKeyPayment(data: IBillKeyPaymentParams): Promise<AxiosPromise<IBillKeyPaymentResponse>> {
        this.makeMainPayBaseInfo({
            mbrNo: data.mbrNo,
            apiKey: data.apiKey,
            amount: data.amount,
     ...
     */
    public async billKeyPaymentCancel(data: IBillKeyPaymentCancelParams): Promise<AxiosPromise<IBillKeyPaymentCancelResponse>> {
        this.makeMainPayBaseInfo({
            mbrNo: data.mbrNo,
            apiKey: data.apiKey,
            amount: data.amount,
     ...
     */
    public async billKeyUnused(data: IBillKeyUnusedParams): Promise<AxiosPromise<IBillKeyUnusedResponse>> {
        this.makeMainPayBaseInfo({
            mbrNo: data.mbrNo,
            apiKey: data.apiKey,
            amount: '0',
     ...
     */
    public async paymentReady(data: IPaymentReadyParams): Promise<IPaymentReadyResponse> {
        this.makeMainPayBaseInfo({
            mbrNo: data.mbrNo,
            apiKey: data.apiKey,
            amount: data.amount,
        ...
        const response: AxiosResponse<IPaymentReadyMainPayResponse> = await axios.post(
     ...
     * @return {}
     */
    public async payment(data: IPaymentParams): Promise<AxiosPromise<IPaymentResponse>> {
        const params = new URLSearchParams();
        params.append('version', '1.0');
        params.append('mbrNo', data.mbrNo);
     ...
     */
    public paymentReadyTokenVerify(token: string): IPaymentReadyTokenCreateParams {
        return jwt.verify(token, process.env.SECRET!) as IPaymentReadyTokenCreateParams;
    };

    /**
     * 신용카드 수기 결제 처리
     */
    public async handwritingPayment(data: IHandwritingPaymentParams): Promise<AxiosPromise<IHandwritingPaymentResult>> {
        this.makeMainPayBaseInfo({
            mbrNo: data.mbrNo,
            apiKey: data.apiKey,
            amount: data.amount,
