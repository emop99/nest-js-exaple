...
export class AuthController {
  ...
  async userLogin(@Body() requestBody: UserLoginApiDto, @Res() res: Response) {
  ...
  async userRegister(@Body() requestBody: UserRegisterApiDto) {
    if (!(await this.authService.idCheck(requestBody.loginId))) {
      throw new UnauthorizedException({ reason: '중복된 아이디 입니다.' });
    }

    try {
      requestBody.password = await this.authService.transformPassword(
