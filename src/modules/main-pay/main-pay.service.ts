...
export class MainPayService {
  ...
  public async billKeyRegister(
    mainPayBillKeyInfo: RegularCardRegisterDto,
  ): Promise<void> {
  ...
  public async billKeyDelete(id: number): Promise<void> {
    await this.mainPayBillKeyRepository.update(
      { id: id },
      { deletedAt: new Date() },
    );
  }

  public async regularPaymentCancelBillKeyDelete(
    userId: number,
    billkey: string,
  ): Promise<void> {
    await this.mainPayBillKeyRepository.update(
      { billKey: billkey, userId: userId },
      { deletedAt: new Date() },
    );
  }

  public async regularPaymentResponseInsert(
    mainPayResponseInfo: RegularCardPaymentResponseDto,
  ): Promise<void> {
    mainPayResponseInfo.failMsg = mainPayResponseInfo.failMsg.replace(
      /"/gi,
      '',
    );
  ...
  public async handwritingPaymentResponseInsert(
    mainPayResponseInfo: HandWritingPaymentResponseDto,
  ): Promise<void> {
    mainPayResponseInfo.failMsg = mainPayResponseInfo.failMsg.replace(
      /"/gi,
      '',
  ...
  public async findPaymentResponse(
    mbrRefNo: string,
  ): Promise<MainPayResponseEntity> {
    return await this.mainPayResponseRepository.findOne({
      where: { mbrRefNo: mbrRefNo },
    });
  }

  public async paymentCancelInfoUpdate(
    cancelInfo: PaymentCancelInfoUpdateDto,
  ): Promise<void> {
    await this.mainPayResponseRepository.update(
      { mbrRefNo: cancelInfo.mbrRefNo },
      {
